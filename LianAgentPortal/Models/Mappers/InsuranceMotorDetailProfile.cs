...
namespace LianAgentPortal.Models.Mappers
{
    public class InsuranceMotorDetailProfile : Profile
    {
        ...
        private string MapInsuranceDetailStatus(InsuranceDetailStatusEnum status)
        {
            ...
            if (status == InsuranceDetailStatusEnum.EVALUATE_PREMIUM_SUCCESS) return "<i class=\"fa fa-check-square\" aria-hidden=\"true\"></i> TÍNH PHÍ - THÀNH CÔNG";
            if (status == InsuranceDetailStatusEnum.EVALUATE_PREMIUM_ERROR) return "<i class=\"fa fa-exclamation-triangle\" aria-hidden=\"true\"></i> TÍNH PHÍ - LỖI";
            if (status == InsuranceDetailStatusEnum.EVALUATE_PREMIUM_INPROGRESS) return "<i class=\"fa fa-spinner\" aria-hidden=\"true\"></i> ĐANG TÍNH PHÍ";
